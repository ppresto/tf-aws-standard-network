...
Run the below command to add this private key to the list maintained by ssh-agent so you're not prompted for it when using SSH or scp to connect to hosts with your public key.
  ...
  ${format("ssh-add %s", module.aws_std_network.private_key_filename)}
...
Use the below command(s) to SSH into a Bastion host using this private key.
  ...
  ${join("\n  ", formatlist("ssh -A -i %s %s@%s", module.aws_std_network.private_key_filename, module.aws_std_network.bastion_username, module.aws_std_network.bastion_ips_public))}

To force the generation of a new key, the private key instance can be "tainted" using the below command.

  terraform taint -module=aws_std_network.ssh_keypair_aws.tls_private_key tls_private_key.key
README
}
...
output "private_key_name" {
  value = "${module.aws_std_network.private_key_name}"
}

output "private_key_filename" {
  value = "${module.aws_std_network.private_key_filename}"
}

output "private_key_pem" {
  value = "${module.aws_std_network.private_key_pem}"
}

output "public_key_pem" {
  value = "${module.aws_std_network.public_key_pem}"
}

output "public_key_openssh" {
  value = "${module.aws_std_network.public_key_openssh}"
}
